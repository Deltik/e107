...
class tinymce4_parseTest extends \Codeception\Test\Unit
{

	/** @var tinymce4_parse $tm */
	private $tm;

	protected function _before()
	{
		require_once(e_PLUGIN . "tinymce4/e_parse.php");
		try
		{
			$this->tm = $this->make('tinymce4_parse');
		}
		catch (Exception $e)
		{
			$this->fail("Couldn't load tinymce4_parse object");
		}
	}

	private function toDB($input)
	{
		$_POST['__meta_type_camelCamelCamel'] = 'bbarea';
		return $this->tm->toDB($input, ['type' => 'string', 'field' => 'camelCamelCamel']);
	}
...
<p>[img class=bbcode-img-left&width=600]{e_MEDIA_IMAGE}2017-11/e107_about.png[/img]Some other text</p>[/html]';
		...
		$test_3 = "&lt;p&gt;Nikdy nehovor, že niečo nejde, pretože sa vždy n&aacute;jde blbec, čo to urob&iacute;.&nbsp;&lt;/p&gt;";
		$actual_3 = $this->toDB($test_3);
		...
		$result3 = $this->tm->toDB($actual_3);
		$this->assertEquals($actual_3, $result3);
	}
	...
	function testtoDBOnScriptTags()
	{
		...
		$result = $this->toDB($string);
		$this->assertSame('[html]' . $string . '[/html]', $result);


		$result = $this->tm->toWYSIWYG($string);
