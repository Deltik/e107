...
class db_verify
{
	...
	{
		return count(array_filter($this->errors, function($tableMetadata)
		{
			return !empty($tableMetadata['_status']);
		}));
	}
			...
			if($tableStatus != self::STATUS_TABLE_OK) // Missing Table
			{
				...
				$text .= "
					<tr>
						...
						<td class='center middle autocheck e-pointer'>".$this->fixForm($this->results[$tabs]['_file'],$tabs, 'all', '', $fixMode) . "</td>
					</tr>
					";		
			}
			foreach($field as $k=>$f)
