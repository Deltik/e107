...
//------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------//
	function render_newsitem($news){
		...
		if($news_title == "Welcome to e107"){
			...
			$category_id = 0;
			$category_icon = e_IMAGE.SITEBUTTON;
		}else{
			$category_icon = (eregi("images", $category_icon) ? THEME.$category_icon : e_IMAGE.$category_icon);
		}
		...
		$news_author = "<a href='user.php?id.".$admin_id."'>".$admin_name."</a>";
		$etext = " <a href='email.php?news.".$news_id."'><img src='".e_IMAGE."generic/friend.gif' style='border:0' alt='email to someone' /></a>";
		$ptext = " <a href='print.php?news.".$news_id."'><img src='".e_IMAGE."generic/printer.gif' style='border:0' alt='printer friendly' /></a>";
		...
		$replace[7] = ($news_allow_comments ? COMMENTOFFSTRING : "<a href='comment.php?$news_id'>".COMMENTLINK.$comment_total."</a>");
