...
if($results = $sql -> db_Select("content", "*", "content_type=0 AND (content_heading REGEXP('".$query."') OR content_subheading REGEXP('".$query."') OR content_content REGEXP('".$query."')) ")){
	while(list($content_id, $content_heading, $content_subheading, $content_content, $content_datestamp, $content_author, $content_comment) = $sql -> db_Fetch()){
		...
		$text .= "<br /><img src='".THEME."images/bullet2.gif' alt='bullet' /> <b><a href='content.php?article.".$content_id."'>".$content_heading_."</a></b> <br />".$content_subheading_.$content_content_;
	}
}else{
	$text .= "No matches.";
}
