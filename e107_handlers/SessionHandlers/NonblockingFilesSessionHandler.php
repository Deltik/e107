...
class NonblockingFilesSessionHandler extends BaseSessionHandler
{
	...
	public function gc($maxlifetime): int|false
	{
		...
		{
			if (!str_starts_with($filename, self::SESSION_FILE_PREFIX))
				continue;
			$path = "{$this->savePath}/$filename";
			$count += $this->expireFile($path, $maxlifetime);
		}
	 ...
	 */
	private function isValidSessionId(string $session_id): bool
	{
		if (
			strlen($session_id) > 128 ||
			str_contains($session_id, '/') ||
			str_contains($session_id, '\\')
		)
		{
			return false;
	 ...
	 * @return int 1 if the file was deleted, 0 if the file was not deleted
	 */
	private function expireFile(string $path, int $maxlifetime): int
	{
		if (filemtime($path) + $maxlifetime < time() && file_exists($path))
		{
			unlink($path);
			return 1;
		}
