...
if($results = $sql -> db_Select("user", "user_id, user_name, user_email, user_homepage, user_signature", "user_name REGEXP('".$query."') OR user_email REGEXP('".$query."') OR user_homepage  REGEXP('".$query."') OR user_signature REGEXP('".$query."') ")){
	while(list($user_id, $user_name, $user_email, $user_homepage, $user_signature) = $sql -> db_Fetch()){
